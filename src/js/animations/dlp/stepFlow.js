...
let userPosition = localStorage.getItem("userPosition") || "setup-1";
...
const redirectUser = (swiper) => {
  if (userScore < 4 && getStageName().toLowerCase() !== "setup") {
    window.location.href = "/dlp/setup";
    return;
  }

  if (userScore > routeThreshold[getStageName().toLowerCase()]) {
    localStorage.setItem(
      "userPosition",
      `${getStageName().toLowerCase()}-${1}`
    );
    return;
  }
  ...
  if (userPosition) {
    if (
      userPosition.split("-")[0].toLowerCase() === getStageName().toLowerCase()
    ) {
      swiper.slideTo(userScore);
      localStorage.setItem(
        "userPosition",
        `${getStageName().toLowerCase()}-${userScore + 1}`
      );
    } else {
      window.location.href = `/dlp/${userPosition.split("-")[0].toLowerCase()}`;
    }
  }
};
...
export const stepFlow = () => {
  ...
  swiperPagination.forEach((btn, index) => {
    btn.addEventListener("click", () => {
      ...
      const stateStepsObj =
        JSON.parse(localStorage.getItem("stepObj")) || stepsObj;
      if (stateStepsObj[`${currentStage}-${index + 1}`].answer) {
